    ...
    <xsl:template match="choice">
        ...
        <xsl:if test="not($FirstChild/self::sic or $FirstChild/self::orig or $FirstChild/self::abbr)">
            <xsl:message terminate="yes">
                ...
                <xsl:value-of select="jmp:XpathOfNode($FirstChild)"/>
            </xsl:message>
        </xsl:if>
        ...
        <xsl:if test="$FirstChild/self::sic and not($SecondChild/self::corr)">
            <xsl:message terminate="yes">
                ...
                <xsl:value-of select="jmp:XpathOfNode($SecondChild)"/>
            </xsl:message>            
        </xsl:if>
            ...
            <xsl:choose>
                ...
                <xsl:when test="self::sic">
                    <span class="sicText" title="{normalize-space(following-sibling::corr)}">
                        <span style="color:red"> <!-- TODO: should match color in recteText class -->
                            <xsl:text>[</xsl:text>
                        </span>
                        <xsl:apply-templates/>        
